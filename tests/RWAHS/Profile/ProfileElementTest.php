...
class ProfileElementTest extends AbstractProfileTest
{
    public function testListAttributesHaveLists()
    {
        ...
        $this->assertEquals(25, $list_elements->length, 'Number of list attributes should match.');
...
LIST_XML
        ...
        foreach ($restrictions as $restriction) {
            ...
            if ($type_element->length) {
                ...
                if ($interstitial) {
                    ...
                    $this->assertEquals(1, $this->xpath->query($selector)->length, "The `$selector` element should exist in the profile. See restriction at: {$restriction->getNodePath()}");
                } else {
                    $this->typeExistsForTable($type, $table, $restriction->getNodePath());
                }
            }
        }
    }
    ...
    public function testListElementsWithNullOptionTextDoNotRequireValue()
    {
        ...
        foreach($optional_list_settings as $optional_list_setting){
            ...
            $this->assertEquals(0, $this->xpath->query($metadata_element->getNodePath() . '/settings/setting[@name="requireValue" and text() = 1]')->length,
                'The element `' . $metadata_element->getAttribute('code') . '` has `nullOptionText` set and therefore should not have a setting of
    ...
    public function testMuseumFieldsExistOnPhotographsAndPublicMemorials()
    {
        ...
        foreach($museum_restrictions as $restriction){
            ...
            if(!in_array($code, array('Description', 'LastEditDate', 'LastEditBy', 'specificLocationMuseum'))){
