...
class ProfileACLTest extends AbstractProfileTest
{
    ...
    {
        $xpath = $this->xpath;
        $this->assertEquals(8, $xpath->query('/profile/groups/group')->length, 'The number of groups should match');
    ...
    {
        $xpath = $this->xpath;
        $this->assertEquals(10, $xpath->query('/profile/roles/role')->length, 'The number of roles should match');
    ...
    {
        $this->assertEquals(42, $this->xpath->query('/profile/roles/role[@code="cataloguer"]/actions/action')->length, 'The cataloguer role should have the correct number of action permissions.');
        $this->assertEquals(2, $this->xpath->query('/profile/roles/role[@code="cataloguer"]/actions/action[php:functionString("preg_match", "/_delete_/", text()) > 0]')->length, 'The cataloguer role should not be able to delete anything.');
    }
}
